/* 박스별 검수 v2 / v11.0.38 (도메인 룰 + LLM 우회 + 빈 list 안전망) */
(async function(){
var VER='v2/v11.0.38';
...
var LLM_ENABLED=!!KEY;
window.__bapDebug={llmCalls:[],errors:[],ver:VER,ruleHits:[]};
...
/* ===== v2 도메인 룰 사전 (LLM 우회) =====
 * 매칭 우선순위: 위에서 아래로 (먼저 매칭되는 룰만 적용)
 * action='NO_CHANGE': 현재 상태 유지, 자동수정 차단 (법령 등 광범위 적합)
 * forceSpaces: 가용 공간 list 중 이 라벨만 강제 (차량/어린이 등 좁은 적합)
 */
var DOMAIN_RULES=[
  {name:'법령/소방',
  ...
  {name:'차량/주차',
   ...
   badge:'🚗 차량용 — 주차/도로만',
   reason:'차량 관련 사인물은 주차/도로 공간에만 적합'},
  {name:'어린이/놀이',
   keywords:/놀이터|미끄럼틀|어린이|유아|보육|화상주의|뜨거움주의|어린이보호|놀이기구/,
   forceSpaces:['놀이터/공원','운동장'],
   badge:'👶 어린이용 — 놀이/교육 공간만',
   reason:'어린이 관련 사인물은 놀이/교육 공간에만 적합'}
];
...
async function buildPlan(s1,s2,name){
  ...
  if(rule){
    ...
    reqs.forEach(req=>{
      ...
      if(rule.action==='NO_CHANGE'){
        ...
        // 가용 공간 list 중 forceSpaces만 강제
        var allOptLabels=p.allItems.map(i=>i.label);
        var finalLabels=rule.forceSpaces.filter(l=>allOptLabels.indexOf(l)>=0&&!isBad(l));
        // 제거: 현재 중 finalLabels에 없는 것
        p.currentItems.forEach(it=>{
          if(finalLabels.indexOf(it.label)<0){p.remove.push(it.label);p.removeItems.push(it);}
        });
        // 추가: finalLabels 중 현재에 없는 것
  ...
  // ===== v2: ④ LLM이 모든 박스 빈 list = "변경 없음" 안전망 =====
  // (소방시설 자체점검 같은 케이스에서 LLM이 전체 DROP 추천하는 사고 방지)
  ...
  if(llmResult){
    ...
    if(resBoxes.length>=2&&emptyBoxes.length===resBoxes.length){
      // 현재 체크 있는 박스가 2개 이상인데 LLM이 전부 빈 list 반환 → 의심
      llmAllEmpty=true;
      console.warn('[bap] LLM이 모든 박스 빈 list 반환 → 변경 없음 모드 (사용자 검토 필요)');
      reqs.forEach(req=>{
        var p=req.plan;
  ...
  window.__bapDebug.llmCalls.push({type:'buildPlan',row:rowDbg});
  return {plans:plans,llmUsed:!!llmResult,llmFailed:llmFailed,llmAllEmpty:llmAllEmpty,indActions:indActions};
}
...
async function fetchAndAnalyze(rgr,name,force){
  try{
    ...
    return {rgr:rgr,name:nm,s1:s1,s2:s2,plans:planResult.plans,actions:actions,
      ...
      llmAllEmpty:!!planResult.llmAllEmpty};
...
if(isE){
  ...
  var failBadge=rd.llmFailed?'<span style="background:#d9534f;color:#fff;padding:3px 8px;border-radius:3px;font-size:12px;margin-left:8px">⚠ LLM 응답 실패 — 자동수정 차단</span>':'';
  var ruleBadgeHdr=rd.ruleApplied?'<span style="background:#ffc107;color:#000;padding:3px 8px;border-radius:3px;font-size:12px;margin-left:8px;font-weight:bold">'+rd.ruleBadge+'</span>':'';
  var emptyBadge=rd.llmAllEmpty?'<span style="background:#6f42c1;color:#fff;padding:3px 8px;border-radius:3px;font-size:12px;margin-left:8px">🛡 LLM 전체 DROP 의심 — 변경 없음 모드</span>':'';
  var blockFix=rd.llmFailed||rd.ruleNoChange||rd.llmAllEmpty;
  ...
  var H='<div class="__bhdr" style="background:#305496;color:#fff;padding:14px 18px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:18px;font-weight:bold">'+ICO[j]+' '+(rd.name||'(없음)')+llmBadge+failBadge+ruleBadgeHdr+emptyBadge+'</div><div style="font-size:13px;opacity:.85">'+rgr+' · '+KOR[j]+' · ❌'+delN+' / ➕'+addN+' · '+VER+'</div></div><div style="display:flex;gap:6px">'+fixBtn+hdrCtrls+'</div></div>';
...
function render(){
  ...
  var llmFailN=res.filter(r=>r.llmFailed).length;
  var ruleN=res.filter(r=>r.ruleApplied).length;
  var allEmptyN=res.filter(r=>r.llmAllEmpty).length;
  H+='<div style="font-size:13px;opacity:.85">'+(CAT_NAMES[cat]||'?')+'('+cat+') / '+pg+'p · ✅'+st.OK+' / ⚠'+st.PARTIAL+' / ❌'+st.EMPTY+(st.MISMATCH?' / 🚫'+st.MISMATCH:'')+(st.ERR?' / err'+st.ERR:'')+' · ❌'+totDel+' ➕'+totAdd+(ruleN?' · <span style="background:#ffc107;color:#000;padding:2px 6px;border-radius:3px">📐 룰적용 '+ruleN+'</span>':'')+(allEmptyN?' · <span style="background:#6f42c1;padding:2px 6px;border-radius:3px">🛡 안전망 '+allEmptyN+'</span>':'')+(llmFailN?' · <span style="background:#d9534f;padding:2px 6px;border-radius:3px">⚠ LLM실패 '+llmFailN+'</span>':'')+'</div></div>';
  ...
  res.forEach((r,i)=>{
    ...
    if(r.llmFailed)H+='<span style="padding:6px 10px;background:#d9534f;color:#fff;border-radius:4px;font-size:11px;font-weight:bold">⚠ LLM실패</span>';
    else if(r.actions.length>0)H+='<button data-fix="'+i+'" class="__brfx" style="padding:6px 12px;background:'+(dN>0?'#d9534f':'#ffc107')+';color:'+(dN>0?'#fff':'#000')+';border:none;border-radius:4px;font-size:12px;cursor:pointer;font-weight:bold">'+(dN>0?'수정+제거':'수정')+' '+r.actions.length+'</button>';
    H+='</div></div>';
  ...
  H+='</div><div style="padding:8px 14px;background:#f5f5f5;font-size:11px;color:#666;border-top:1px solid #ddd">박스마다 [현재 / ❌제거 / ➕추가 / ⇒결과] · 이중 LLM + LOCK 캐시 + 5xx재시도(v11.0.37)<span style="float:right">'+VER+'</span></div>';
  p.innerHTML=H;attachCtrls();
  ...
  Array.from(p.querySelectorAll('.__brfx')).forEach(b=>{
    b.onclick=async function(){
      ...
      if(r.llmFailed){alert('LLM 응답 실패한 행은 자동수정 불가. 다시 검수 후 재시도.');return;}
