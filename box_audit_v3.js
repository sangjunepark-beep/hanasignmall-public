/* 박스별 검수 v3 / v11.0.39 (forceSpaces 보수화 + 부분비움 안전망 + 복합룰 + 라벨 동의어) */
(async function(){
var VER='v3/v11.0.39';
...
async function buildPlan(s1,s2,name){
  ...
  window.__bapDebug.llmCalls.push({type:'buildPlan',row:rowDbg});
  return {plans:plans,llmUsed:!!llmResult,llmFailed:llmFailed,llmAllEmpty:llmAllEmpty,llmManyEmpty:llmManyEmpty,indActions:indActions};
}
...
if(isE){
  ...
  var emptyBadge=rd.llmAllEmpty?'<span style="background:#6f42c1;color:#fff;padding:3px 8px;border-radius:3px;font-size:12px;margin-left:8px">🛡 LLM 전체 DROP 의심 — 변경 없음</span>':'';
  var manyEmptyBadge=rd.llmManyEmpty?'<span style="background:#6f42c1;color:#fff;padding:3px 8px;border-radius:3px;font-size:12px;margin-left:8px">🛡 LLM 50%↑ DROP 의심 — 변경 없음</span>':'';
  var blockFix=rd.llmFailed||rd.ruleNoChange||rd.llmAllEmpty||rd.llmManyEmpty;
...
function render(){
  ...
  var allEmptyN=res.filter(r=>r.llmAllEmpty||r.llmManyEmpty).length;
  ...
  res.forEach((r,i)=>{
    ...
    else if(r.ruleNoChange)H+='<span style="padding:6px 10px;background:#ffc107;color:#000;border-radius:4px;font-size:11px;font-weight:bold">📐 룰차단</span>';
    else if(r.llmAllEmpty||r.llmManyEmpty)H+='<span style="padding:6px 10px;background:#6f42c1;color:#fff;border-radius:4px;font-size:11px;font-weight:bold">🛡 안전망</span>';
    else if(r.ruleApplied&&r.actions.length>0)H+='<button data-fix="'+i+'" class="__brfx" style="padding:6px 12px;background:#ffc107;color:#000;border:none;border-radius:4px;font-size:12px;cursor:pointer;font-weight:bold">📐 룰수정 '+r.actions.length+'</button>';
  ...
  if(ball)ball.onclick=async function(){
    ...
    for(var i=0;i<allRgrs.length;i++){
      ...
      var pgr=document.getElementById('__bpr');if(pgr)pgr.textContent='검수 '+newRes.length+' / '+allRgrs.length+' (v3)';
    }
  ...
  if(bfa)bfa.onclick=async function(){
    if(!confirm('전체 자동수정?\n❌ '+totDel+' + ➕ '+totAdd+'\n(LLM 실패 / 룰 차단 / 안전망 행은 자동 제외)'))return;
